...
public class MapOverlayCreator {
	...
	func loadTextToArray() {
		...
		for lat in 0 ..< latArray.count {
			...
			let newDisease = Disease(lat: latitude, long: longitude, diseaseName: name, date: date)
			self.datapoints.append(newDisease)
		}
		self.toUseDatapoints = datapoints
	}
	...
	func filterDate(date: Date) {
		toUseDatapoints = datapoints
		var shift = 0
		for i in 0 ..< toUseDatapoints.count {
			if toUseDatapoints[i - shift].date > date {
				toUseDatapoints.remove(at: i - shift)
				shift = shift + 1
			}
		}
	}
